...
v7.0 — антигаллюцинационный протокол, Verifier, уровни в торговом плане.
v7.1 — пост-фильтр report_sanitizer на ответах; FinBERT NEUTRAL/MIXED без «медвежьего уклона»
       в BEAR_SYSTEM; слои A/B/C и ужесточение SYNTH/VERIFIER; см. main.py для дебатов/кэша.
"""
...
from report_sanitizer import sanitize_agent_output

logger = logging.getLogger(__name__)


def _sanitize_reply(text: str) -> str:
    cleaned, _ = sanitize_agent_output(text)
    return cleaned
...
Нельзя говорить "FinBERT подтверждает бычий/медвежий исход", если в блоке NEUTRAL или только MIXED без явного BULLISH/BEARISH.

СЛОИ ДАННЫХ (не смешивай):
- СЛОЙ A — только цифры из «РЕАЛЬНЫЕ РЫНОЧНЫЕ ДАННЫЕ»
- СЛОЙ B — факты из новостей: цитируй формулировку из контекста
- СЛОЙ C — сценарии «если»: помечай «ГИПОТЕЗА:» или «если произойдёт X»
"""

ANTI_HALLUCINATION_RULE = """

🚫 ЖЁСТКИЙ АНТИГАЛЛЮЦИНАЦИОННЫЙ ФИЛЬТР:

1. ЛЮБАЯ ЦИФРА БЕЗ ИСТОЧНИКА = ❌ ГАЛЛЮЦИНАЦИЯ
   Формат: (Источник: Binance/Yahoo/FRED/Alpha Vantage/Finnhub)

2. ЗАПРЕЩЕНО:
   - "7 из 10 случаев"
   - "исторически X%"
   - "по данным аналитиков"
   - "ожидается", "консенсус", "эксперты считают"

3. ЗАПРЕЩЕНО использовать источники, которых нет в контексте:
   ❌ CoinDesk, ARK, JPMorgan, Reuters, FT и т.д.

4. ЗАПРЕЩЕНО придумывать исторические аналогии:
   ❌ "как в 2020"
   ❌ "аналогично прошлому циклу"

5. ЕСЛИ НЕ УВЕРЕН — НЕ ПИШИ ЦИФРУ

6. Verifier имеет право удалить любой аргумент с пометкой:
   ❌ ГАЛЛЮЦИНАЦИЯ [УДАЛИТЬ]

7. Synth ОБЯЗАН игнорировать такие аргументы полностью
...
6. «халвинг Ethereum», «EIP-4844», «Dencun», даты хардфорков ETH — ЗАПРЕЩЕНЫ, если этой формулировки нет дословно в блоке НОВОСТЕЙ
...
- Сравнивать VIX/рынок с 2020, 2022, «историческими» уровнями без цитаты из контекста
""" + COMMON_GROUNDING_RULE + ANTI_HALLUCINATION_RULE
...
- FinBERT MIXED или NEUTRAL → "тональность заголовков не даёт перевеса ни быкам, ни медведям — осторожность и меньший размер позиции; не приравнивать нейтральность к медвежьему тренду рынка"
...
д) «исторически», год 2020–2023 в прошедшем времени, «халвинг Ethereum», «EIP-4844», «Dencun» без дословной цитаты из блока НОВОСТЕЙ → ❌ ГАЛЛЮЦИНАЦИЯ [УДАЛИТЬ]
...
"🔬 FINBERT: [score] → [BULLISH/BEARISH/MIXED/NEUTRAL] | Уверенность: [HIGH/MEDIUM/LOW]
 Влияние на вердикт: [как FinBERT повлиял на итоговый вес валидных аргументов]"

ОСОБО — FinBERT NEUTRAL или MIXED (в т.ч. с уверенностью EXTREME/HIGH):
- НЕ пиши "FinBERT подтверждает медвежий/бычий исход". Нейтральность = нет направления в заголовках, не доказательство тренда.
- Вес FinBERT ниже макро (VIX, нефть, геополитика из новостей) и ниже цен активов из контекста.
- Допустимая формулировка: "FinBERT NEUTRAL/MIXED: заголовки не дают перевеса ни быкам, ни медведям."
...
"[валидный аргумент ✅] + при NEUTRAL/MIXED FinBERT не используй как «перевес»; при BULLISH/BEARISH укажи как FinBERT согласуется с макро и ценами"
...
1b. FinBERT NEUTRAL или MIXED без явного BULLISH/BEARISH → не усиливай ни бычий, ни медвежий итог; опирайся на макро и цены из контекста
...
class DebateOrchestrator:
    ...
    async def run_debate(
        ...
        bull_r1, bear_r1 = _sanitize_reply(bull_r1), _sanitize_reply(bear_r1)
        ...
        if rounds >= 2:
            ...
            verify_r2 = _sanitize_reply(
                await self.verifier.respond(full_context, history, round_num=2)
            )
            history.add(f"{self.verifier.emoji} {self.verifier.name}", verify_r2, 2)
            bull_r2 = _sanitize_reply(
                await self.bull.respond_counter(full_context, history, round_num=2)
            )
        ...
        if rounds >= 3:
            ...
            bear_r3 = _sanitize_reply(
                await self.bear.respond_counter(full_context, history, round_num=3)
            )
            history.add(f"{self.bear.emoji} {self.bear.name}", bear_r3, 3)

        # Доп раунды
        for extra_round in range(4, rounds + 1):
            bull_x = _sanitize_reply(
                await self.bull.respond_counter(full_context, history, extra_round)
            )
            history.add(f"{self.bull.emoji} {self.bull.name}", bull_x, extra_round)
            bear_x = _sanitize_reply(
                await self.bear.respond_counter(full_context, history, extra_round)
            )
            history.add(f"{self.bear.emoji} {self.bear.name}", bear_x, extra_round)

        logger.info("Финальный синтез (только валидные аргументы)...")
        final_synthesis = _sanitize_reply(
            await self.synth.respond(full_context, history, round_num=rounds)
        )
