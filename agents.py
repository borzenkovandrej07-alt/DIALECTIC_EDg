...
2. МАКРО → ставки/инфляция/доллар → логическое следствие для рынков
   ВАЖНО: CPI ~326 это индекс, не процент. Пиши "инфляция ~3-4% годовых", не "CPI 326.59".
...
3. МАКРО → инфляция > 4% = ФРС не снизит ставки → давление на рост-активы
   ВАЖНО: CPI это индекс (~326), не процент инфляции.
   Инфляция = изменение CPI год к году (~3-4%).
   ВСЕГДА пиши для читателя понятно: "инфляция ~3.2% годовых" а не "CPI 326.59".
   Если видишь CPI 326.59 — переведи в процент сам или напиши "инфляция умеренная (~3-4%)".
...
ОСОБО ВАЖНО — ПРОГНОЗЫ ИЗ НОВОСТЕЙ:
Если агент цитирует прогноз из новости (например "нефть вырастет до $120") —
это НЕ факт, это чьё-то мнение. Помечай явно:
"⚠️ ПРОГНОЗ, НЕ ФАКТ — [источник] предсказывает [X], но текущая цена [Y].
Прогноз не верифицирован, используй с осторожностью."
...
- Принимать прогнозы из новостей как факты ("нефть вырастет до $120" — это мнение, не факт)
- Писать "CPI 326.59" — переводи в "инфляция ~3-4% годовых" для читателя
"""
