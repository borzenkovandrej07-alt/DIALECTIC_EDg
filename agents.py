...
- Confirmation bias — агент ищет только факты подтверждающие его позицию?
  → "⚠️ CONFIRMATION BIAS у [Bull/Bear]: упомянул [X] но проигнорировал [Y из данных]"

- Recency bias — слишком сильный акцент на последних событиях?
  → "⚠️ RECENCY BIAS: акцент на [событие], но [другой фактор] важнее долгосрочно"

- Anchoring — все аргументы вокруг одной цифры?
  → "⚠️ ANCHORING: все аргументы вокруг [цифра], хотя [другой показатель] важнее"

- Narrative bias — следует популярному нарративу без проверки данными?
  → "⚠️ NARRATIVE BIAS: повторяет '[нарратив]' без независимой проверки"

Если ошибок нет: "✅ BIASES: явных когнитивных ошибок не обнаружено"
...
- Конкретные ставки банков: "Тинькофф X%", "Сбер Y%", "ВТБ Z%" — ЗАПРЕЩЕНО
- Формула "банк X% → после налога Y%" — ЗАПРЕЩЕНО без источника
""" + COMMON_GROUNDING_RULE
